...
3. **Broken `packages.default` after migration (2026-07-20)**: The vendored→referenced migration left a duplicate `default` attribute in `packages` (one referencing undefined `mkCavekitPlugin`, another containing a misplaced devShell definition referencing undefined `lefthookWrappersFor`). Nix refused to evaluate the flake. Fixed by replacing with a proper `runCommand` derivation that builds the cavekit plugin from `cavekit-src` via `install-plugin.sh`.
4. **Missing `config/lefthook/file_size_limits.yml` (2026-07-20)**: The `base` fragment's `file-size-check` check requires this config file, but the migration did not create it. Fixed by adding the config with limits matching the set-and-setting ecosystem convention.
5. **Missing `.nix-embedded-shell-allowlist` (2026-07-20)**: The `nix-no-embedded-shell` check flagged `flake.nix` for the `writeShellApplication` `text` block in `apps.confirm`, which inherently requires Nix-interpolated shell. Fixed by adding `flake.nix` to the allowlist, following the set-and-setting convention.
